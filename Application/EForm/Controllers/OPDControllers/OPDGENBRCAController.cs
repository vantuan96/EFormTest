...
{
    [SessionAuthorize]
    public class OPDGENBRCAController : BaseApiController
    {
        private readonly string visit_type = "OPD";

        [HttpGet]
        [Route("api/OPD/OPDGENBRCA/Info/{type}/{visitId}")]
        [Permission(Code = "GENBRCAGET")]
        ...
        [HttpGet]
        [Route("api/OPD/OPDGENBRCA/{type}/{visitId}")]
        [Permission(Code = "GENBRCAGET")]
        public IHttpActionResult GetNCCNBROV1(Guid visitId, string type = "A01_201_201119_V")
        {
            ...
            if (form == null)
                ...
                {
                        ViMessage = "Phiếu đánh giá tiêu chuẩn chỉ định xét nghiệm GEN BRCA không tồn tại",
                        EnMessage = "Criteria Assesssment for GEN BRCA Test does not exist",
                        IsLocked24h = Is24hLocked(visit.CreatedAt, visitId, type, user.Username)
                });           
        ...
        [HttpPost]
        [Route("api/OPD/OPDGENBRCA/Create/{type}/{visitId}")]
        [Permission(Code = "GENBRCAPOST")]
        public IHttpActionResult CreateOPDGENBRCA(Guid visitId,string type = "A03_119_201119_V")
        {
            var visit = GetVisit(visitId, visit_type);
            if (visit == null)
            ...
            if (form != null)
                return Content(HttpStatusCode.BadRequest,
                    new
                    {
                        ViMessage = "Phiếu đánh giá tiêu chuẩn chỉ định xét nghiệm GEN BRCA đã tồn tại",
                        EnMessage = "Criteria Assesssment for GEN BRCA Test already exist"
                    });

            var form_data = new OPDGENBRCA()
            {
                VisitId = visitId
            };
            unitOfWork.OPDGENBRCARepository.Add(form_data);
        ...
        [HttpPost]
        [Route("api/OPD/OPDGENBRCA/Update/{type}/{visitId}")]
        [Permission(Code = "GENBRCAPOST")]
        public IHttpActionResult UpdateNCCNBROV1(Guid visitId,[FromBody] JObject request,string type = "A03_119_201119_V")
        {
            var visit = GetVisit(visitId, visit_type);
            if (visit == null)
                return Content(HttpStatusCode.BadRequest, Message.OPD_NOT_FOUND);
            ...
            var islock24h = Is24hLocked(visit.CreatedAt, visitId, type, user.Username, form.Id);         
            ...
            unitOfWork.OPDGENBRCARepository.Update(form);
        ...
        [HttpPost]
        [Route("api/OPD/OPDGENBRCA/Confirm/{type}/{visitId}")]
        //[Permission(Code = "IPDGENASS3")]
        public IHttpActionResult ConfirmAPI(Guid visitId, [FromBody] JObject request, string type = "A03_119_201119_V")
        {
            var visit = GetVisit(visitId, visit_type);
            if (visit == null)
                return Content(HttpStatusCode.BadRequest, Message.OPD_NOT_FOUND);
        ...
        private OPDGENBRCA GetForm(Guid visit_id)
        {
            return unitOfWork.OPDGENBRCARepository.Find(e => !e.IsDeleted && e.VisitId == visit_id).FirstOrDefault();
        }
        private dynamic FormatOutput(string formCode, OPD opd, OPDGENBRCA fprm)
        {
            var UserConfirm = unitOfWork.UserRepository.FirstOrDefault(u => !u.IsDeleted && u.Id == fprm.UserConfirmId);
            ...
            return new
            {
                ...
                IsLocked24h = Is24hLocked(opd.CreatedAt, opd.Id, formCode, user.Username),
                Confirm = new
                {
                    User = new
                    {
                        ...
                        ConfirmAt = fprm.UserConfirmAt,
                    }
                },
                IsLocked = Is24hLocked(opd.CreatedAt, opd.Id, formCode, user.Username),
            };
        }
        private bool ConfirmUser(OPDGENBRCA opdGENBRCA, User user, string kind)
        {
            var positions = user.PositionUsers.Where(e => !e.IsDeleted).Select(e => e.Position.EnName.ToUpper());
            if (opdGENBRCA.UserConfirmId == null)
            {
                opdGENBRCA.UserConfirmAt = DateTime.Now;
                opdGENBRCA.UserConfirmId = user?.Id;
            }
            else
            {
                return false;
            }
            unitOfWork.OPDGENBRCARepository.Update(opdGENBRCA);
            unitOfWork.Commit();
            return true;
        }
