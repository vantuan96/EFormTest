...
{
    [SessionAuthorize]
    public class IPDObservationChartController : BaseOPDApiController
    {
        [HttpGet]
        [Route("api/IPD/ObservationChart/{id}")]
        [Permission(Code = "IPDOB01")]
        public IHttpActionResult GetObservationChart(Guid id)
        {
            using (var txn = GetNewReadUncommittedScope())
            {
                ...
                var observation_chart = visit.ObservationChart;
                if (observation_chart == null || observation_chart.IsDeleted)
                {
                    ...
                    visit.OPDObservationChartId = oc.Id;
                    unitOfWork.Commit();
                    observation_chart = oc;
                }
        ...
        [HttpGet]
        [Route("api/IPD/ObservationChart/Chart/{id}")]
        [Permission(Code = "IPDOB01")]
        public IHttpActionResult GetChart(Guid id)
        {
            ...
            var observation_chart = opd.ObservationChart;
            if (observation_chart == null || observation_chart.IsDeleted)
