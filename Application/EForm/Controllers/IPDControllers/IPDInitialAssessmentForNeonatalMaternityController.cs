...
{
    public class IPDInitialAssessmentForNeonatalMaternityController : BaseIPDApiController
    {
        private readonly string visit_type = "IPD";
        private const string timeUpdate_version2 = "HIDE_A02_016_050919_VE";

        [HttpGet]
        [Route("api/IPD/InitialAssessment/ForNeonatalMaternityV2/{type}/{ipdId}/{tab}/{*id}")]
        [Permission(Code = "IPDVIEWMATERNITYV2")]
        ...
        [HttpPost]
        [Route("api/IPD/InitialAssessment/ForNeonatalMaternityV2/Create/{type}/{ipdId}/{tab}/{*formIdNewborn}")]
        [Permission(Code = "IPDCREATEMATERNITYV2")]
        ...
        [HttpPost]
        [Route("api/IPD/InitialAssessment/ForNeonatalMaternityV2/Update/{type}/{ipdId}/{tab}/{*id}")]
        [Permission(Code = "IPDUPDATEMATERNITYV2")]
        public IHttpActionResult UpdateAPI(string type, Guid ipdId, string tab, [FromBody] JObject request, Guid? id)
        {
            ...
            if (IsVisitLastTimeUpdate(visit, timeUpdate_version2) && form.Version == 2)
                if (CheckSpecialtiesIsObstetrics(visit, form))
                {
        ...
        [HttpPost]
        [Route("api/IPD/InitialAssessment/ForNeonatalMaternityV2/Confirm/{type}/{ipdId}/{tab}/{*id}")]
        [Permission(Code = "IPDCONFIRMMATERNITYV2")]
        public IHttpActionResult ConfirmAPI(string type, Guid ipdId, string tab, [FromBody] JObject request, Guid? id)
        {
            ...
            var successConfirm = ConfirmUser(form, user, request["TypeConfirm"].ToString(), "IPDCONFIRMMATERNITYV2");
            if (successConfirm)
            {
                UpdateVisit(visit, visit_type);
        ...
        [HttpGet]
        [Route("api/IPD/InitialAssessment/ForNeonatalMaternityV2/GetList/{visitId}")]
        [Permission(Code = "IPDVIEWMATERNITYV2")]
        public IHttpActionResult GetListFormVersion2(Guid visitId)
        {
            ...
            return Content(HttpStatusCode.OK, new
            {
                IsLocked = IPDIsBlock(visit, "A02_016_050919_VE"),
        ...
        [HttpGet]
        [Route("api/IPD/InitialAssessment/ForNeonatalMaternityV2/InforVersion/{visitId}")]
        public IHttpActionResult APICheckVersion(Guid visitId)
        {
            IPD visit = GetIPD(visitId);
            if (visit == null)
                return Content(HttpStatusCode.NotFound, Message.IPD_NOT_FOUND);

            string setupMedicalRecords = CheckCodeSetupMedicalRecords(visit);

            return Content(HttpStatusCode.OK, new { IsNewVisit = IsVisitLastTimeUpdate(visit, timeUpdate_version2), MedicalRecordCode = setupMedicalRecords });
        }
        ...
        private string CheckCodeSetupMedicalRecords(IPD visit)
        {
            var form = (from e in unitOfWork.IPDInitialAssessmentForNewbornsRepository.AsQueryable()
                        where !e.IsDeleted && e.VisitId == visit.Id
                        && e.Version == 2 && (e.DataType.ToUpper() == "ForNeonatalMaternity_Obstetrics".ToUpper() || e.DataType.ToUpper() == "ForNeonatalMaternity".ToUpper())
                        select e.DataType).ToList();
            if (form.Count == 0)
            {
                string[] code_medicalRecords = { "A01_035_050919_V", "A01_038_050919_V" };
                var check_setup = (from e in unitOfWork.IPDSetupMedicalRecordRepository.AsQueryable()
                                   where !e.IsDeleted && e.SpecialityId == visit.SpecialtyId
                                   && code_medicalRecords.Contains(e.Formcode)
                                   && e.FormType.ToUpper() == "MedicalRecords".ToUpper()
                                   && e.IsDeploy == true
                                   select e.Formcode).ToList();
                if (check_setup.Count == 0)
                    return "";
                if (check_setup.Contains("A01_035_050919_V"))
                    return "A01_035_050919_V";

                return "A01_038_050919_V";
            }

            if (form.Contains("ForNeonatalMaternity_Obstetrics"))
                return "A01_035_050919_V";
        ...
        private List<dynamic> GetListForms(Guid visitId)
        {
            var forms = (from e in unitOfWork.IPDInitialAssessmentForNewbornsRepository.AsQueryable()
                         ...
                         where !e.IsDeleted && e.VisitId == visitId && e.DataType.ToUpper() == "ForNeonatalMaternity_Obstetrics".ToUpper()
                         && n.VisitId == visitId && n.FormCode.ToUpper() == "A02_016_050919_VE_ForNeonatalMaternity_Obstetrics".ToUpper()
        ...
        private bool CheckSpecialtiesIsObstetrics(IPD visit, IPDInitialAssessmentForNewborns form)
        {
            const string code_obstetrics = "A01_035_050919_V";
            var specialty_id = visit.SpecialtyId;
            var setup_obstetrics = (from e in unitOfWork.IPDSetupMedicalRecordRepository.AsQueryable()
                                    where !e.IsDeleted && e.SpecialityId == specialty_id
                                    && e.Formcode.ToUpper() == code_obstetrics.ToUpper()
                                    && e.FormType.ToUpper() == "MedicalRecords".ToUpper()
                                    && e.IsDeploy == true
                                    select e).FirstOrDefault();
            if (setup_obstetrics == null)
            {
                var form_obstetrics = (from e in unitOfWork.IPDInitialAssessmentForNewbornsRepository.AsQueryable()
                                       where !e.IsDeleted && e.VisitId == visit.Id
                                       && e.DataType.ToUpper() == "ForNeonatalMaternity_obstetrics".ToUpper()
                                       select e).FirstOrDefault();
                if (form_obstetrics == null)
                    return false;
