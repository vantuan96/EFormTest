...
namespace EForm.Common
{
    public class EHosClient
    {
        ...
        {
            string url_postfix = string.Format("/searchPatientByName?HO_TEN={0}", name);
            var response = RequestForSearchPatients(url_postfix);
            if (response != null)
            {
                return BuildResult(response);
        ...
        public static List<dynamic> SearchCustomerByNameAndDOB(string name, string dob)
        {
            ...
            string url_postfix = string.Format("/searchPatientByNameAndDob?HO_TEN={0}&NGAY_SINH={1}", name, date_of_birth);
            var response = RequestForSearchPatients(url_postfix);
            if (response != null)
            {
                return BuildResult(response);
        ...
        {
            string url_postfix = string.Format("/searchPatientByNameAndYob?HO_TEN={0}&NAM_SINH={1}", name, yob);
            var response = RequestForSearchPatients(url_postfix);
            if (response != null)
            {
                return BuildResult(response);
        ...
        {
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", Constant.EHOS_API_SERVER_TOKEN);
            client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            ServicePointManager.ServerCertificateValidationCallback += (sender, cert, chain, sslPolicyErrors) => true;
            string url = string.Format("{0}{1}", Constant.EHOS_API_SERVER_URL, url_postfix);
            try
            {
        ...
        private static List<dynamic> BuildResult(JToken customer_data)
        {
            ...
            foreach (JToken customer in customer_data)
            {
                result.Add(new
                {
                    ...
                    DateOfBirth = FormatDOB(customer.Value<string>("NgaySinh")),
